...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (!user) {
    console.log(`Failed login attempt due to unregistered email :  ${email}. TimeStamp: ${new Date().toLocaleString()}`)
  ...
  if (!authValid) {
    console.log(`Failed login attempt due to incorrect password but registered Email :  ${email}. TimeStamp: ${new Date().toLocaleString()}`)
    return res.status(401).send({auth: false, message: 'Password was invalid.'});
  }

  const jwt = generateJWT(user);
  console.log(`Successful login by user with Email :  ${email}. TimeStamp: ${new Date().toLocaleString()}`)
